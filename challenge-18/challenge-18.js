(function () {
	...
	*/
	...
	var fraseMeses = 'Os meses de janeiro, junho e julho começam com a letra j.';
	var regexMeses = /ju\w+/g;
	console.log(fraseMeses.match(regexMeses));
	...
	*/
	...
	var regexHTML = /<\w+>/g;
	console.log(fraseHTML.match(regexHTML));
	...
	*/
	...
	var fraseHTMLVazio = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';
	var regexHTMLVazio = /<\w+><\/\w+>/g;
	console.log(fraseHTMLVazio.match(regexHTMLVazio));
	console.log(fraseHTMLVazio.replace(regexHTMLVazio, function (regex, texto) {
		return texto;
	}));
	...
	*/
	...
	var regexTexto = /(<)(\w+)(>)([^<]+)(<\/\w+>)/gi;

	var textoHTML = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
	console.log(textoHTML.match(regexTexto));

	console.log(textoHTML.replace(regexTexto, function (captura, abertura, tag, fechamento, texto, finalTag) {
		return abertura + tag + fechamento + 'O texto dentro da tag ' + tag + ' é "' + texto + '"' + finalTag + '\n';
	}));
