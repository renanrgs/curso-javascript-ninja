(function (win, doc) {
	...
	*/
	...
	$button.addEventListener('click', function (event) {
		...
		var arrayErrors = [];
		arrayErrors.push(isCampoValido($inputUsername, 'Preencha o nome!'));
		arrayErrors.push(isCampoValido($inputEmail, 'Prrencha o e-mail!'));
		arrayErrors.push(isCampoValido($message, 'Preencha a mensagem!'));
		arrayErrors.push(isValidEmail($inputEmail));

		var hasErrors = arrayErrors.some(function (currentValue, index) {
			return currentValue === undefined;
		});

		hasErrors ? alert('Não enviado.') : confirm('Tem certeza que deseja enviar o formulário?');
	}, false);


	function isCampoValido($elemento, mensagem) {
		return $elemento.value ? true : alert(mensagem);
	}
	...
	*/
	function isValidEmail($email) {
		var regex = /^[\w+.]+@\w+\.\w{2,}(?:\.\w{2,2})?$/gm
		return $email.value.match(regex) ? true : alert('Entre com um e-mail válido!');
	}
})(window, document);
